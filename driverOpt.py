...
comboPoints = np.zeros(21346) # NB these numbers will need changing if the threshold values are changed
comboNames  = [[] for i in range(21346)]
numberRanIt = 100
np.random.seed(0)
for randomIndex in range(numberRanIt):
    print "Iteration Index %i: %i%% complete"%(randomIndex,100.*(float(randomIndex)/float(numberRanIt)))
...
comboPoints    = np.array(comboPoints)/numberRanIt
